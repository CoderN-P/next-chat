...
export default function ChatMessage({message=null, author=null, group=false, newMessage} : {message?: Message|null, author?: User|null, group?: boolean, newMessage: boolean}){
    ...
    if (message){
        ...
        if (EmojiPattern.test(updatedMessageContent)) {
            updatedMessageContent = twemoji.default.parse(updatedMessageContent, {folder: "svg", ext: ".svg", className: "emojiWrapper2"});
        } else {
            updatedMessageContent = twemoji.default.parse(updatedMessageContent, {folder: "svg", ext: ".svg", className: "emojiWrapper1"});

        }
        updatedMessageContent = marked.marked(updatedMessageContent);
        if (updatedMessageContent) {
            updatedMessageContent = DOMPurify.sanitize(updatedMessageContent, {ADD_ATTR: ['target']});
            sanitizedHtml = parse(updatedMessageContent) as string;
    ...
    if (newMessage){
        className = "flex flex-row group px-1 w-full dark:bg-blue-500/10 items-center dark:hover:bg-neutral-900";
    }
