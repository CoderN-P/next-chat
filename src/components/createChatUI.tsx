  ...
  return (
    <div className={className}>
      <div className="flex flex-row mb-2 justify-between items-center">
        ...
        <input onInput={(e) => setInputText((e.target as HTMLTextAreaElement).value)} type="text" className=" flex-1 w-4/5 rounded-md border-l dark:border-neutral-800 p-2 dark:bg-neutral-950 dark:hover:bg-neutral-900" placeholder="Enter a name"/>
            {submitting ? <Spinner/> : <><button onClick={() => createChatClient(inputText)} disabled={!inputText}><svg className="w-6 h-6 mx-2 text-green-400 dark:hover:text-green-400 dark:text-green-300" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" stroke="currentColor">
